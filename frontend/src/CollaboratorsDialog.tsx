...
// Vendored from office rather than imported: modules never import each other.
// Forms has no "comment" level — a form is either readable or editable.
const PERMS: CollabPermission[] = ['edit', 'view']
        ...
        <div ref={boxRef} className="relative">
          ...
          {open && debounced.length > 0 && (
            <div className="absolute z-10 left-0 right-0 mt-1 bg-white border border-border rounded shadow-lg max-h-60 overflow-y-auto">
              {filtered.length === 0
                ? <div className="px-3 py-2 text-xs text-text-tertiary">{t('share_no_results', 'Aucun utilisateur')}</div>
                : filtered.map(r => (
            ...
            <div className="space-y-1">
              {data?.owner && (
                <div className="flex items-center gap-2 py-1.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-xs truncate">{data.owner.display_name || data.owner.email}</div>
              ...
              {(data?.collaborators ?? []).map(c => (
                <div key={c.user_id} className="flex items-center gap-2 py-1.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-xs truncate">{c.display_name || c.email}</div>
              ...
              {(data?.collaborators ?? []).length === 0 && (
                <div className="text-xs text-text-tertiary py-1">{t('share_only_you', 'Vous seul·e avez accès')}</div>
              )}
            </div>
          )}
        </div>
