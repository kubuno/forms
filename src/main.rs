...
#[derive(Parser, Debug)]
#[command(name = "kubuno-forms", version, about = "Module formulaires Kubuno")]
struct Cli {
    #[arg(short, long, env = "KFRM_CONFIG_FILE")]
...
async fn main() -> Result<()> {
    ...
    match settings.logging.format {
        kubuno_forms::config::LogFormat::Json   => subscriber.json().init(),
        kubuno_forms::config::LogFormat::Pretty => subscriber.init(),
    }

    tracing::info!("Kubuno Forms v{} démarrage…", env!("CARGO_PKG_VERSION"));
    ...
    tokio::fs::create_dir_all(&settings.storage.local_path)
        .await
        .ok();
    tokio::fs::create_dir_all(&settings.storage.temp_path)
        .await
        .ok();
    ...
    // Heartbeat toutes les 30s
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let url    = format!("{}/internal/modules/forms/heartbeat", settings2.core.url);
    ...
    tracing::info!("Kubuno Forms démarré sur http://{addr}");
...
async fn register_with_core(http: &Client, settings: &Settings) {
    ...
    let display_name  = manifest.as_ref().map(|m| m.module.display_name.as_str()).unwrap_or("Forms").to_string();
    ...
    let sidebar_items: Vec<Value> = manifest.as_ref()
        ...
        .unwrap_or_else(|| vec![
            json!({ "id": "forms", "label": "Formulaires", "icon": "ClipboardList", "path": "/forms", "position": 45 }),
        ]);
    ...
    for attempt in 1u32.. {
            ...
            .await
        {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Module forms enregistré auprès du core");
                return;
            }
