...
- **Choice of database engine.** Forms now runs on **PostgreSQL**,
  **MySQL/MariaDB** or **SQLite** — the administrator picks one in
  ...
  whichever is named; all three drivers are built in. SQLite needs no server
  (`[database] path` holds `forms.sqlite`), which makes a single-box or offline
  install trivial. Migrations ship per engine and a portability test suite runs
  the whole data path against each one.

### Changed

- **Database access rewritten on the shared `kubuno-db` foundation.** Every
  query goes through the engine-agnostic layer instead of talking to PostgreSQL
  directly, so behaviour is identical on the three engines: primary keys are
  generated by the module (no reliance on a database default), `RETURNING`,
  `ON CONFLICT`, JSON operators (including the `theme ? 'fontFamily'` key test of
  the default-typeface migration), `ILIKE`, aggregates and `INET` are each
  translated to the local dialect, and the response-count and `updated_at`
  triggers exist on all three engines. No visible change on an existing
  PostgreSQL instance — the migrations it already applied are untouched.
...
- The recipient search and the collaborator listing read the core's
  `core.users` table directly; that only resolves when the module shares a
  database with the core (PostgreSQL, or MySQL on the same server), not on a
  standalone SQLite deployment. Storage-usage byte figures on MySQL/SQLite are
  the logical length of the stored values rather than PostgreSQL's on-disk size.
